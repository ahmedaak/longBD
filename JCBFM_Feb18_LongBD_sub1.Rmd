...
This is a comprehensive report of all the results of the "Longitudinal BOLD delay" manuscript, submitted  to *JCBFM* in April 2018.
...
```{r Supp_Figure_3, message=F, cache=F, warning=F, echo=F, fig.width=8, fig.height=8}
# Load in perfusion values
...
# define subjects with and without recanalization in entire sample
...
par(mfrow=c(2,2), oma=c(0,2,0,0))
                    ...
                    ############ NON-RECANALIZERS ################
# pre-allocate vectors for effect sizes in non-recanalizers 
allrois_es_allsubs_nonrecan <- c()
allrois_names_nonrecan <- c()

# ROIs to be displayed in the plots
...
for(x in text_filenames){
  ...
  for(i in subjects_nonrecan) {
    ...
    if(file.exists(roi_es_path_nonrecan)){
      # read effect size file (pre-calculated - takes a long time)
      load(roi_es_path_nonrecan)
      # concatenate effect sizes across subjects into one vector
...
# BOLD delay - healthy hemisphere reference
allrois_es_allsubs_nonrecan_long_HH <- melt(allrois_es_allsubs_nonrecan[,4:6], id.vars =allrois_names_nonrecan)
# create boxplots
boxplot(value ~ Var2, data = allrois_es_allsubs_nonrecan_long_HH, cex.axis = 0.7, outline = F, xaxt = "n", main = "", ylim = c(-0.5, 0.7), xlab = "ROI", ylab = "Effect size", yaxt = "n",cex.lab=1.5) 
# add individual points
...
mtext(text = "B", side = 3, line = 0.5, adj = -0.15, font = 2, cex=1.5)
                    ...
                    ################## RECANALIZERS ##################
# pre-allocate vectors for effect sizes in recanalizers 
allrois_es_allsubs_recan <- c()
allrois_names_recan <- c()

# loop through ROIs to be displayed in plots
for(x in text_filenames)
{
...
# create boxplots
boxplot(value ~ Var2, data = allrois_es_allsubs_recan_long_VS, cex.axis = 0.7, outline = F, xaxt = "n", main = "", ylim = c(-0.5, 0.7), xlab = "ROI", ylab = "Effect size", yaxt = "n",cex.lab=1.5) 
# add individual points
...
# create boxplots
boxplot(value ~ Var2, data = allrois_es_allsubs_recan_long_HH, cex.axis = 0.7, outline = F, xaxt = "n", main = "", ylim = c(-0.5, 0.7), xlab = "ROI", ylab = "Effect size", yaxt = "n",cex.lab=1.5) 
# add individual points
