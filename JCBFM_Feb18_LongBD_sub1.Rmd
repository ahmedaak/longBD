...
data_path <- c("S:/AG/AG-CSB_NeuroRad/fillebvb/DATA_PROCESSING/")

knitr::opts_chunk$set(echo = TRUE, dev = c("png","pdf"), fig.path = paste(data_path,"Figures/",sep=""))
...
demo_allsubs <- read.csv(file = paste(data_path,"demographics_by_group.csv", sep = ""))
...
# load data table with all the volumes
    volumes_data <- read.csv2(file=paste(data_path,"volumes.csv",sep=""), header = T, na.strings = "NaN", stringsAsFactors = F, sep = ";", dec = ".")
...
datadir <- paste(data_path, "mean_ROI_vals_aff_VT",sep="")
...
dice_allsubs <- read.csv(paste(data_path,"dice_coeff_r.csv",sep=""))
...
# load perfusion values within affected VT
  BD_tmax_aff <- readMat(paste(data_path,"roivals_affVT_tsa_tmax_perf.mat",sep="")) # BOLD delay and Tmax
      ...
      for(x in text_filenames){
       ...
       # loop through subjects
                ...
                # check if file exists
                  roi_es_path_nonrecan <- paste(data_path, "voxelwise_vals/",i,"/",x,"_es.Rdata", sep = "")
          ...
          for(x in text_filenames)
          {
           ...
           assign(roi_es_allsubs_recan, c())
                  # loop through subjects
                        ...
                        # check if file exists
                          roi_es_path_recan <- paste(data_path, "voxelwise_vals/",i,"/",x,"_es.Rdata", sep = "")
...
```{r Supp_Figure_6, message=F, cache=F, warning=F, echo=F, fig.width=6, fig.height=6}
    ...
    setwd(paste(data_path,"cross_corr_all/",sep=""))
    dwi_maxcc_all <- c()
    cl_maxcc_all <- c()
    tsa_maxcc_all <- c()
...
```{r Repository_plots, message=F, cache=F, warning=F, echo=F, fig.width=10, fig.height=8}
    # subject names
...
allrois_names <- c()
      ...
      for (x in text_filenames) 
        ...
        perf_d0_file <- paste(data_path,"voxelwise_vals/", i, "/" , x , "_D0.txt" , sep = "")
        perf_d1_file <- paste(data_path,"voxelwise_vals/", i, "/", x , "_D1.txt", sep = "")
      ...
      # load results of tests for differences between D0 + D1 in each of the ROIs
              ...
              if(length(perf_d0)>1 && length(perf_d1)>1){
                          load(paste(data_path, "voxelwise_vals/",i, "/", x, "_individual_differences_d0_d1_ROIs.Rdata", sep = ""))
      
      # calculate effect size for D0/D1 comparison
                  perf_d0_d1_es <- perf_d0_d1_wilcox@statistic@teststatistic/sqrt(length(perf_d0)*2)
                  save(perf_d0_d1_es, list = c("perf_d0_d1_es"), file = paste(data_path, "voxelwise_vals/",i, "/", x, "_es.Rdata", sep = ""))
